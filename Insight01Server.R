...
output$Histogram__1_ <- renderPlotly({
  plot = dfA1() %>% ggplot() + 
    geom_histogram(mapping = aes(x=deaths, colour = sex, fill = sex), binwidth = 8500) +
    ...
    ggtitle(paste(Sex(), input$title__1_)) +
  ...
  if( ! is.na(input$plotWidth__1_) & ! is.na(input$plotHeight__1_))
    ggplotly(plot, tooltip = c("deaths"), session="knitr", width = input$plotWidth__1_, height = input$plotHeight__1_)
  else
